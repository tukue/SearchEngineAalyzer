...
import { fetchWithNetworkLimits, validatePublicHttpsUrl, createHttpError } from "../../../server/url-safety";
import { getTenantContextOrError } from "../_lib/auth";
import { ensureNextApiEnabled } from "../_lib/feature-flags";

export async function POST(request: NextRequest) {
  const disabledResponse = ensureNextApiEnabled("analyze");
  ...
  try {
    ...
    console.error("Failed to parse analyze body:", error);
    return NextResponse.json({ message: "Invalid JSON body" }, { status: 400 });
  }

  const requestId = requestBody.requestId || UsageLimitsService.generateRequestId();
  const auditType = requestBody.auditType || "meta_analysis";
  const url = requestBody.url;
  const userId = requestBody.userId;

  let quotaReservation: Awaited<ReturnType<typeof UsageLimitsService.reserveAuditQuota>>;
  try {
    quotaReservation = await UsageLimitsService.reserveAuditQuota(
    ...
    return NextResponse.json({ message: "Failed to check quota" }, { status: 500 });
  }

  if (!quotaReservation.success) {
    return NextResponse.json(
      { ...quotaReservation.error, quota: quotaReservation.quotaStatus },
      { status: 429 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({ ...storedAnalysis, quota: quotaReservation.quotaStatus });
  } catch (error) {
    try {
    ...
    if (error instanceof z.ZodError) {
      ...
      return NextResponse.json({ message: validationError.message || "Invalid request format" }, { status: 400 });
    }

    if ((error as any).status) {
      return NextResponse.json({ message: (error as Error).message }, { status: (error as any).status });
    }
